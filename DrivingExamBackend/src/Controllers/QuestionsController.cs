...
namespace DrivingExamBackend.Controllers
{
    ...
    public class QuestionsController : ControllerBase
    {
        ...
        [HttpGet("/api/exam/{moduleGuid}")]
        [ProducesResponseType<List<QuestionDto>>(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        public async Task<ActionResult<List<QuestionDto>>> GetRandomQuestions([FromRoute] Guid moduleGuid)
        {
            var questionsQuery = _db.Questions
                .Where(q => q.Module.Guid == moduleGuid)
                .Include(q => q.Answers);

            var totalQuestionsCount = await questionsQuery.CountAsync();
            if (totalQuestionsCount == 0)
            {
                return NotFound("No questions found for this module.");
            }

            var takeCount = Math.Min(20, totalQuestionsCount);

            var allQuestionIds = await questionsQuery.Select(q => q.Guid).ToListAsync();

            var random = new Random();
            var selectedQuestionIds = allQuestionIds
                .OrderBy(x => random.Next())
                .Take(takeCount)
                .ToList();

            var selectedQuestions = await _db.Questions
                .Where(q => selectedQuestionIds.Contains(q.Guid))
                .Select(q => new QuestionDto(
                    q.Guid, q.Number, q.Text, q.Points, q.ImageUrl, q.Module.Guid, q.Topic.Guid,
                    q.Answers.Select(a => new AnswerDto(a.Guid, a.Text)).ToList()))
                .ToListAsync();
